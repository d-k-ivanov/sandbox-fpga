...
Use only the necessary pragmas to make the architecture fully iterative (reducing area and increasing latency), using the code with ternary operators as base.

3. Latency Reduction (Parallelization)
Use only the necessary pragmas to try and reduce the latency of the code with ternary operators by a factor of 4 (in practice, it will be slightly less). Describe which results indicate that this parallelization has indeed been performed.
